...
export function ApiKeysSettings() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || 'Não foi possível carregar as chaves de API');
        return;
      }
  ...
  async function handleRevoke(key: ApiKey) {
    ...
    try {
      ...
      if (!res.ok) {
        const payload = await res.json().catch(() => ({}));
        toast.error(payload.error || 'Não foi possível revogar a chave');
        return;
      }
...
function CreateKeyDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        toast.error(payload.error || 'Não foi possível criar a chave');
        return;
      }
        ...
        {createdKey ? (
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                Esta é a única vez que a chave completa é exibida. Guarde-a em um
                lugar seguro — se você a perder, revogue-a e crie uma nova.
              </DialogDescription>
            </DialogHeader>
          ...
          <>
            <DialogHeader>
              ...
              <DialogDescription className="text-muted-foreground">
                Nomeie-a conforme a integração que a usará e conceda apenas
