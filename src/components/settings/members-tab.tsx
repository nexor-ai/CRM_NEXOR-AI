...
const EDITABLE_ROLES: { value: AccountRole; label: string; hint: string }[] = [
  ...
  { value: 'viewer', label: 'Visualizador', hint: 'Somente leitura no aplicativo' },
];
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || 'Não foi possível atualizar o papel');
        return;
      }
        ...
        <div>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-muted-foreground">
              A URL do convite em texto puro é exibida apenas uma vez na criação,
              por segurança — para compartilhar de novo, revogue o convite abaixo e
              crie um novo.
            </p>
          ) : null}
