...
export function AiKnowledgeCard({
  ...
  const fetchDocs = useCallback(async () => {
    ...
    try {
      ...
      else toast.error(data.error ?? 'Não foi possível carregar a base de conhecimento');
    } catch {
  ...
  const openEdit = async (id: string) => {
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        toast.error(data.error ?? 'Não foi possível abrir o documento');
        return;
      }
      setEditing(id);
      setTitle(data.title ?? '');
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Adicione FAQs, políticas ou detalhes do produto. O assistente busca os
          trechos relevantes ao redigir e responder automaticamente, para que
          ele possa responder em vez de transferir.
          {hasEmbeddingsKey
            ? ' A busca semântica está ativada (chave de embeddings definida).'
              ...
              <div className="space-y-3 rounded-md border border-border p-3">
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Cole a resposta da FAQ, o texto da política ou detalhes do produto…"
                    rows={8}
                    disabled={saving}
                  />
                </div>
