/**
 * Handlers for local preset's template-lifecycle webhook events.
 *
 * local preset delivers three template-related webhook fields, each with a
 ...
 *   - message_template_components_update  — local preset auto-modified the template
 *
 ...
 * These fields are NOT subscribed to by default. In local preset App Dashboard
 ...
 * done, status updates only land via the manual "Refresh presets"
 ...
 * defensively in case local preset adds new template fields later.
 */
...
async function handleStatusUpdate(
  ...
  // where local preset sends a human-readable explanation. Clear it on any
  // other status flip so the UI doesn't show a stale REJECTED banner
  // after local preset re-approves a resubmitted template.
  const update: Record<string, unknown> = {
    status,
    rejection_reason:
      status === 'REJECTED' ? value.reason ?? 'Rejected by local preset' : null,
...
/**
 * local preset auto-modified the template (typically a category reclassification
 * — e.g. Marketing → Utility after content review).
 *
 * For v1 we just log and let the user pull updated components via the
 * existing "Refresh presets" button — persisting local preset's modified
 ...
 * "local preset modified this template" banner.
 */
function handleComponentsUpdate(value: TemplateComponentsUpdateValue): void {
  console.info(
    '[template-webhook] components updated by local preset for template',
    value.message_template_id,
    value.message_template_name,
    '— run "Refresh presets" in Settings to pull the new components.',
  )
}
