...
describe('generateReply — OpenRouter', () => {
  it('uses the OpenRouter endpoint and attribution headers', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      okResponse({
        choices: [{ message: { content: 'Resposta via OpenRouter' } }],
      })
    );
    vi.stubGlobal('fetch', fetchMock);
