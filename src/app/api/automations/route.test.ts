...
const mocks = vi.hoisted(() => {
  ...
  insertBuilder.single.mockResolvedValue({
    data: { id: 'automation-1', account_id: 'account-1', user_id: 'viewer-1' },
    error: null,
  })
...
vi.mock('@/lib/automations/steps-tree', () => ({
  buildStepRows: vi.fn(() => [{ id: 'server-step', parent_step_id: null }]),
}))

import { POST } from './route'

function request() {
  return new Request('https://crm.test/api/automations', {
    method: 'POST',
...
describe('POST /api/automations authorization', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('creates parent and steps through one account-scoped RPC', async () => {
    mocks.requireRole.mockResolvedValueOnce({ userId: 'agent-1', accountId: 'account-1' })
    mocks.rpc.mockResolvedValueOnce({ data: { id: 'automation-1' }, error: null })
    ...
    expect(mocks.rpc).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        p_steps: [{ id: 'server-step', parent_step_id: null }],
      }),
    )
  })
})
