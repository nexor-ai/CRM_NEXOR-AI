...
describe('agents API auth', () => {
  ...
  it('returns 403 before any mutation for a viewer', async () => { mocks.requireRole.mockRejectedValue(new mocks.ForbiddenError('Insufficient role')); const response = await POST(new Request('https://crm.test/api/ai/agents', { method: 'POST', body: '{}' })); expect(response.status).toBe(403); });
});
