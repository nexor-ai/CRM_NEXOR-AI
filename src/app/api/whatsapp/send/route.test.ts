...
vi.mock('@/lib/auth/account', () => ({
  ...
  toErrorResponse: (error: unknown) =>
    Response.json(
      ...
      {
        status:
          error instanceof authMocks.UnauthorizedError
            ? 401
            : error instanceof authMocks.ForbiddenError
              ? 403
              : 500,
      }
    ),
...
describe('POST /api/whatsapp/send — contact_id template path', () => {
  ...
  it('rejects malformed interactive payloads before any transport call', async () => {
    const res = await POST(
      new Request('http://localhost/api/whatsapp/send', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    expect(res.status).toBe(400);
    expect((await res.json()).error).toMatch(/3 buttons/i);
    expect(sendTemplateMessage).not.toHaveBeenCalled();
  });

  it('rejects a viewer before parsing or calling the WhatsApp transport', async () => {
    authMocks.requireRole.mockRejectedValueOnce(
      new authMocks.ForbiddenError('This action requires the agent role')
    );
