...
const nextConfig: NextConfig = {
  // Production is promoted to an isolated directory before the service
  // restarts. Regular `next build` runs keep using `.next`, so validating a
  // candidate on 3011 can no longer replace chunks/CSS under the live 3010
  // process.
   ...
   *   - Everything else — private + no-store. The same pathname can
   *     return authenticated app HTML, an auth redirect, or a signed-in
   *     redirect away from /login. A shared edge cache must never reuse
   *     one of those responses for another session. This also prevents
   *     stale HTML/RSC from referencing chunks removed by a later deploy.
   *
